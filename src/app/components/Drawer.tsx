...
import { fecharDrawer, removerItem, setEtapa } from "../store/carrinhoSlice";
import { FormEntrega } from "./checkout/FormEntrega";
import { FormPagamento } from "./checkout/FormPagamento";
import { ConfirmacaoPedido } from "./checkout/ConfirmacaoPedido";
import {
  Overlay,
  Panel,
  PanelHeader,
...
export function Drawer() {
  ...
  const { items, drawerAberto, etapa } = useSelector((s: RootState) => s.carrinho);
  ...
  return (
    <>
      ...
      <Panel>
            ...
            {items.length === 0 ? (
              ...
              <>
                ...
                <Rodape>
                  ...
                  <FinalizarButton type="button" onClick={() => dispatch(setEtapa("entrega"))}>
                    Finalizar pedido
                  </FinalizarButton>
                </Rodape>
              </>
        ...
        {etapa === "entrega" && <FormEntrega />}
        {etapa === "pagamento" && <FormPagamento />}
        {etapa === "confirmacao" && <ConfirmacaoPedido />}
      </Panel>
    </>
  );
}
